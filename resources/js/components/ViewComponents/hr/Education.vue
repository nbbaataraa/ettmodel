...
<div>
    ...
    <v-data-table :headers="headers" :items="edu" :search="searchTable" class="elevation-1">
        ...
        <template v-slot:no-results>
            <v-alert :value="true" color="error" icon="warning">Таны хайсан "{{ searchTable }}" утганд ѳгѳгдѳл алга.</v-alert>
        </template>
    </v-data-table>
    <v-layout>
        <v-flex xs12 sm6 md4>
            <!-- Add Auto Complate -->
            <v-autocomplete v-model="select" :loading="loading" :items="items" item-text="employee.lname" item-value="id" :search-input="search" label="Enter Name: "></v-autocomplete>
